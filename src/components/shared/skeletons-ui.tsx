...
export function CardSkeleton({ children }: { children?: React.ReactNode }) {
    return (
        <div className={`${shimmer} border relative overflow-hidden rounded-xl bg-gray-100 p-2 shadow-sm`}>
            <div className="flex p-4">
                <div className="h-2 w-2 rounded-md bg-gray-200" />
                <div className="ml-2 h-4 w-10 rounded-md bg-gray-200 text-sm font-medium" />
            </div>
            {children}
...
export function CardDataSkeleton() {
    return (
        <div className={shimmer + " border bg-white rounded-lg"}>
            <div className="flex justify-between items-center w-full h-full p-3 ">
                <dl className="space-y-3">
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        <div className="bg-gray-300 animate-pulse h-4 w-32 rounded"></div>
                    </dt>
                    <dd className="text-2xl font-light md:text-4xl dark:text-white">
...
export function CardsSkeleton({ amount = 3 }: { amount?: number } = {}) {
    return (
        <>
            {Array.from({ length: amount }).map((_, index) => (
                <CardDataSkeleton key={index} />
            ))}
        </>
    );
}
